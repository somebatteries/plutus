...
testScripts = "v1-scripts" `testWith` evalScripts
  where
      ...
      evalScripts p = isRight . runExcept . Scripts.evaluateScript $ Script (mkProg p) defaultPIRprog
...
evalAPI pv t =
    ...
    let s :: SerializedScript = BSS.toShort . BSL.toStrict . CBOR.serialise $ Script (mkProg t) defaultPIRprog
