...
typeOfBuiltinFunction
    :: forall uni fun. ToBuiltinMeaning uni fun => BuiltinVersion fun -> fun -> Type TyName uni ()
...
Besides having no need for this currently, it complicates the codebase since the typechecker now
pointlessly wants to know the builtin-version before typechecking. To alleviate this, we use the
'Default.def' builtin version during typechecking / lifting. @effectfully: the solution to the
problem would be to establish what kind of backwards compatibility we're willing to maintain and
