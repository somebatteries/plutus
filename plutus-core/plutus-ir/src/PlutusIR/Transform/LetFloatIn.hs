...
floatInBinding ::
    ...
    Term tyname name uni fun (a, Uniques) ->
    Term tyname name uni fun (a, Uniques)
floatInBinding ver letAnn = go True
  where
    go ::
        Bool ->
        Binding tyname name uni fun (a, Uniques) ->
        Term tyname name uni fun (a, Uniques) ->
        Term tyname name uni fun (a, Uniques)
    go canFloatIntoLambdas b !body = case b of
        ...
        TermBind (_, usBind) _strictness var _rhs -> case body of
            Apply (a, usBody) fun arg
                | (var ^. PLC.unique) `Set.notMember` termUniqs fun ->
                    ...
                    Apply (a, Set.union usBind usBody) fun (go canFloatIntoLambdas b arg)
                | (var ^. PLC.unique) `Set.notMember` termUniqs arg ->
                    -- `arg` does not mention `var`, so we can place the binding inside `fun`.
                    Apply (a, Set.union usBind usBody) (go canFloatIntoLambdas b fun) arg
            LamAbs (a, usBody) n ty lamAbsBody | canFloatIntoLambdas ->
                ...
                LamAbs (a, Set.union usBind usBody) n ty (go canFloatIntoLambdas b lamAbsBody)
            TyAbs (a, usBody) n k tyAbsBody | canFloatIntoLambdas ->
                -- A lazy binding, or a strict binding whose RHS is pure, can always
                -- be placed inside a `TyAbs`.
                -- See Note [Float bindings into lambda abstractions].
                TyAbs (a, Set.union usBind usBody) n k (go canFloatIntoLambdas b tyAbsBody)
            TyInst (a, usBody) tyInstBody ty ->
                ...
                TyInst (a, Set.union usBind usBody) (go canFloatIntoLambdas b tyInstBody) ty
            Let (a, usBody) r bs letBody
                | (var ^. PLC.unique) `Set.notMember` foldMap bindingUniqs bs ->
                    ...
                    -- inside `letBody`.
                    Let (a, Set.union usBind usBody) r bs (go canFloatIntoLambdas b letBody)
                ...
                , TermBind (a', usBind') s' var' rhs' <- b' ->
                    ...
                    let b'' = TermBind (a', Set.union usBind usBind') s' var' (go False b rhs')
            ...
            IWrap (a, usBody) ty1 ty2 iwrapBody ->
                ...
                IWrap (a, Set.union usBind usBody) ty1 ty2 (go canFloatIntoLambdas b iwrapBody)
            Unwrap (a, usBody) unwrapBody ->
                -- A binding can always be placed inside an `Unwrap`.
                Unwrap (a, Set.union usBind usBody) (go canFloatIntoLambdas b unwrapBody)
            _ ->
                let us = Set.union (termUniqs body) (bindingUniqs b)
                 in Let (letAnn, us) NonRec (pure b) body
        _ ->
